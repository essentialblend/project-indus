...
void Indus::run()
{
  ...
  world->addWorldObj(makePrim(Point3f{ Float(0),Float(-1000),Float(0) }, Float(1000), ground));
  ...
  {
    const auto& [c, r, m] = h;
    world->addWorldObj(makePrim(c, r, m));
    placed.emplace_back(c, r);
  }
  ...
  auto collide = [&](const Point3f& c, Float r) 
  {
    for (auto& pr : placed) 
    {
      ...
      if (std::sqrt(dx * dx + dz * dz) < r + pr.second + Float(0.05)) 
        return true;
    } 
    return false;
  };
  ...
  for (int i = 0; i < 220; ++i) 
  {
    ...
    Float a = -Float(std::numbers::pi_v<float>) + Float(2) * Float(std::numbers::pi_v<float>) * rf();
    ...
    if (rf() < 0.72f) 
    {
      auto idx = std::min<std::size_t>(palette.size() - 1, std::size_t(rf() * palette.size()));
      world->addWorldObj(makePrim(c, rr, std::make_shared<Diffuse>(palette[idx])));
    }
    else 
    {
      Float eta = Float(1.25) + Float(0.5) * rf();
      world->addWorldObj(makePrim(c, rr, std::make_shared<MDielectric>(ColorRGB{ 1 }, ColorRGB{ 1 }, Float(1.0), eta)));
    }
  }

  //auto world{ std::make_unique<WorldObjectList>() };

  //Transform4f worldFromObject = Transform4f::translate({ 0, 0, 0 });

  //Transform4f renderFromObject =
  //  m_camera->getCameraTransform().getRenderFromWorld() * worldFromObject;

  //Transform4f objectFromRender{ renderFromObject.getInv(), renderFromObject.get() };

  //auto red = std::make_shared<Diffuse>(ColorRGB{ 1, 0, 0 });
  //auto s = std::make_shared<Sphere>(renderFromObject, objectFromRender,
  //  false, 1.0f, -1.0f, 1.0f, 360.0f);
  //world->addWorldObj(std::make_unique<GeometricPrimitive>(s, red));
  ...
  m_film->writeImage(m_cfg, renderTimer);
}
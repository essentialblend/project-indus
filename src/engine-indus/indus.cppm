...
void Indus::renderShirleyFirstBookScene()
{
	...
	m_mainWorld.addWorldObj(std::make_unique<WOSphere>(Point3(0, -1000, 0), 1000, groundMat));

	for (int a = -10; a < 10; a++) {
		for (int b = -10; b < 10; b++) {
			auto matChosen = UGenRNG<double>();
			Point3 center(a + 0.9 * UGenRNG<double>(), 0.2, b + 0.9 * UGenRNG<double>());

			if ((center - Point3(4, 0.2, 0)).getMagnitude() > 0.9)
			{
	...
	m_mainWorld.addWorldObj(std::make_unique<WOSphere>(Point3(-1.3, 1.1, 0), 1.1, firstMat));
...
void Indus::renderFresnelTestScene() 
{
	...
	m_mainWorld.addWorldObj(
		std::make_unique<WOSphere>(Point3(0, -1000, 0), 1000, groundMat));
...
void Indus::renderLiTestScene()
{
	...
	Point3 sphereCenter = camPos + camDir * 5.0;
	m_LiTestWorld.addWorldObj(std::make_unique<WOSphere>(sphereCenter, 1.0, groundMat));
	m_LiTestWorld.addWorldObj(std::make_unique<WOSphere>(Point3(0, 0, 0), 0.5, groundMat));
}

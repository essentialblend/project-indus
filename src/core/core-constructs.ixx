...
export struct CameraProperties
{
	...
	Point3 camLookFrom{ -15, 2.25, 8 };
	Point3 camCenter{ camLookFrom };
	Point3 camLookAt{ 0, 0, 0 };
	Vec3 camVUP{ 0, 1, 0 };

	Vec3 camW{ getUnit(camLookFrom - camLookAt) }; Vec3 camU{ getUnit(computeCross(camVUP, camW)) }; Vec3 camV{ computeCross(camW, camU) };

	void setLookFromAt(const Point3& lookFrom, const Point3& lookAt)
	{

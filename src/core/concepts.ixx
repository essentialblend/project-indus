export module indus.core.concepts;
...
export
{
  ...
  template<typename T>
  concept IntervalScalarLike =
    requires {
    requires FloatingArithmetic<decltype(std::declval<T>().getLower())>;
    requires FloatingArithmetic<decltype(std::declval<T>().getUpper())>;
  };

  template<typename T>
  concept FloatScalarLike = FloatingArithmetic<T> && (!IntervalScalarLike<T>);
  ...
  template<std::size_t N>
  concept Arity23 = Arity2<N> || Arity3<N>;

  template<std::size_t N>
  concept Arity234 = Arity23<N> || Arity4<N>;

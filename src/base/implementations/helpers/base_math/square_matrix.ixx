...
public:
    ...
    constexpr std::span<const T> operator[](unsigned int) const noexcept;
    constexpr std::span<T> operator[](unsigned int) noexcept;
...
export template <typename ResultType, Arithmetic T, std::size_t N>
...
// SquareMatrix member function implementations. Needs to be in the interface as its templated.
template <Arithmetic T, std::size_t N>
constexpr SquareMatrix<T, N>::SquareMatrix() noexcept {
    for (std::size_t i{}; i < N; ++i) {
        for (std::size_t j{}; j < N; ++j) {
            m_memberMatrix[i][j] = (i == j) ? static_cast<T>(1) : static_cast <T>(0);
        }
    }
}
...
template <Arithmetic T, std::size_t N>
constexpr std::span<const T> SquareMatrix<T, N>::operator[](unsigned int i) const noexcept {
    return std::span<const T>(m_memberMatrix[i], N);
}

template <Arithmetic T, std::size_t N>
constexpr std::span<T> SquareMatrix<T, N>::operator[](unsigned int i) noexcept {
...
constexpr SquareMatrix<T, N>& SquareMatrix<T, N>::operator/=(const T& scalar) noexcept
{
    if (scalar == 0) {
        for (unsigned int i = 0; i < N; ++i) {
            for (unsigned int j = 0; j < N; ++j) {
                m_memberMatrix[i][j] = 0;  // Handle zero division by setting matrix to zero
            }
        }
    }
    else {
...
template<typename ResultType, Arithmetic T, std::size_t N>

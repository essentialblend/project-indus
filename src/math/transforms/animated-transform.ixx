...
Ray AnimatedTransform::operator()(const Ray& r) const noexcept
{
  ...
  const Transform4f T{ interpolate(t) };

  Point3fi rayOriginInterval{ T(Point3fi{ r.getOrigin() }) };
  Vec3f rayDirection{ T(r.getDirection()) };

  const Float lengthSq{ computeDot(rayDirection, rayDirection) };

  if (lengthSq > Float{})
  {
    const Vec3f absd{ std::abs(rayDirection[0]), std::abs(rayDirection[1]), std::abs(rayDirection[2]) };
    
    const Float dt{ computeDot(absd, rayOriginInterval.getError<Float>()) / lengthSq };

    const Vector<Interval<Float>, 3> dI{ rayDirection };
    const Interval<Float> dtI{ dt };

    // The tMax idea is TBD, CRITICAL!
    rayOriginInterval = rayOriginInterval + (dI * dtI);
  }

  return Ray{ Point3f{ rayOriginInterval }, rayDirection, r.getTimeSeconds() };
}

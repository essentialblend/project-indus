export module statsaccumulator;
...
private:
  ...
  static double toMB(std::uint64_t bytes);
};
...
void StatsAccumulator::reset(std::size_t maxThreads) noexcept
{
  ...
  if (maxThreads > s_counters.size())
  {
    s_counters.resize(maxThreads);
  }

  for (auto& c : s_counters)
  {
    c = RenderStats{};
  }
}
...
RenderStats StatsAccumulator::finalize() noexcept
{
  ...
  for (const auto& c : getAllCounters())
  {
    ...
    {
      total.nodesVisited.min =
        std::min(total.nodesVisited.min, c.nodesVisited.min);
      total.nodesVisited.max =
        std::max(total.nodesVisited.max, c.nodesVisited.max);
    }
...
double StatsAccumulator::toMB(std::uint64_t bytes)
{
